...
public class OpTimes extends AbstractBinaryOperator implements IAritOperator {

	public OpTimes(AbstractElement a_left, AbstractElement a_right)
	...
	protected ValueStmt do_eval() throws SMVParseException {
		...
		if (lc | rc) {
			try {
				...
				ValueIntStmt other = lc ? r.toValueIntStmt() : l
						.toValueIntStmt();
				return lc ? ValueIntStmt.TimesOpWithCons(int_val, other)
						: ValueIntStmt.TimesOpWithCons(other, int_val);
			} catch (NumberFormatException nfe) {
				throw new SMVParseException("Caanot do aritmatic operation "
						+ "on string values.");
			}
		...
		return ValueIntStmt.TimesOp(l.toValueIntStmt(), r.toValueIntStmt());
	}
	...
	@Override
	protected OpTimes duplicate() throws SMVParseException {
		return new OpTimes(this.getLeft(), this.getRight());
	}
	// ////////////////////////////////////////////////////////////////////////
	// ////////////////////////////////////////////////////////////////////////
	// ////////////////////////////////////////////////////////////////////////
	...
	protected ValueStmt doDefaultNodeOp(ValueStmt lv, ValueStmt rv)
			throws SMVParseException {
		return ValueIntStmt.TimesOp(lv.toValueIntStmt(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doBDD2ConsRangeNode(ValueBDDStmt lv,
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.TimesOpWithCons(lv.toValueIntStmt(), rv.fromRange());
	}
	...
	protected ValueStmt doBDD2ConsStrNode(ValueBDDStmt lv, ValueConsStrStmt rv)
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.TimesOpWithCons(lv.toValueIntStmt(), new Integer(
					rv.getValues()[0]));
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
	...
	protected ValueStmt doConsRange2BDDNode(ValueConsRangeStmt lv,
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.TimesOpWithCons(lv.fromRange(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doConsStr2BDDNode(ValueConsStrStmt lv, ValueBDDStmt rv)
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.TimesOpWithCons(new Integer(lv.getValues()[0]),
					rv.toValueIntStmt());
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
