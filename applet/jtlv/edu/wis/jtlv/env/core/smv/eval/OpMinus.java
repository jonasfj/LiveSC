...
public class OpMinus extends AbstractBinaryOperator implements IAritOperator {

	public OpMinus(AbstractElement a_left, AbstractElement a_right)
	...
	protected ValueStmt do_eval() throws SMVParseException {
		...
		if (lc | rc) {
			try {
				...
				ValueIntStmt other = lc ? r.toValueIntStmt() : l
						.toValueIntStmt();
				return lc ? ValueIntStmt.MinusOpWithCons(int_val, other)
						: ValueIntStmt.MinusOpWithCons(other, int_val);
			} catch (NumberFormatException nfe) {
				throw new SMVParseException("Caanot do aritmatic operation "
						+ "on string values.");
			}
		...
		return ValueIntStmt.MinusOp(l.toValueIntStmt(), r.toValueIntStmt());
	}
	...
	@Override
	protected OpMinus duplicate() throws SMVParseException {
		return new OpMinus(this.getLeft(), this.getRight());
	}
	...
	protected ValueStmt doDefaultNodeOp(ValueStmt lv, ValueStmt rv)
			throws SMVParseException {
		return ValueIntStmt.MinusOp(lv.toValueIntStmt(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doBDD2ConsRangeNode(ValueBDDStmt lv,
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.MinusOpWithCons(lv.toValueIntStmt(), rv.fromRange());
	}
	...
	protected ValueStmt doBDD2ConsStrNode(ValueBDDStmt lv, ValueConsStrStmt rv)
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.MinusOpWithCons(lv.toValueIntStmt(), new Integer(
					rv.getValues()[0]));
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
	...
	protected ValueStmt doConsRange2BDDNode(ValueConsRangeStmt lv,
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.MinusOpWithCons(lv.fromRange(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doConsStr2BDDNode(ValueConsStrStmt lv, ValueBDDStmt rv)
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.MinusOpWithCons(new Integer(lv.getValues()[0]),
					rv.toValueIntStmt());
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
