...
public class OpPlus extends AbstractBinaryOperator implements IAritOperator {

	public OpPlus(AbstractElement a_left, AbstractElement a_right)
	...
	protected ValueStmt do_eval() throws SMVParseException {
		...
		if (lc | rc) {
			try {
				...
				ValueIntStmt other = lc ? r.toValueIntStmt() : l
						.toValueIntStmt();
				return lc ? ValueIntStmt.PlusOpWithCons(int_val, other)
						: ValueIntStmt.PlusOpWithCons(other, int_val);
			} catch (NumberFormatException nfe) {
				throw new SMVParseException("Caanot do aritmatic operation "
						+ "on string values.");
			}
		...
		return ValueIntStmt.PlusOp(l.toValueIntStmt(), r.toValueIntStmt());
	}
	...
	@Override
	protected OpPlus duplicate() throws SMVParseException {
		return new OpPlus(this.getLeft(), this.getRight());
	}
	...
	protected ValueStmt doDefaultNodeOp(ValueStmt lv, ValueStmt rv)
			throws SMVParseException {
		return ValueIntStmt.PlusOp(lv.toValueIntStmt(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doBDD2ConsRangeNode(ValueBDDStmt lv,
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.PlusOpWithCons(lv.toValueIntStmt(), rv.fromRange());
	}
	...
	protected ValueStmt doBDD2ConsStrNode(ValueBDDStmt lv, ValueConsStrStmt rv)
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.PlusOpWithCons(lv.toValueIntStmt(), new Integer(
					rv.getValues()[0]));
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
	...
	protected ValueStmt doConsRange2BDDNode(ValueConsRangeStmt lv,
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.PlusOpWithCons(lv.fromRange(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doConsStr2BDDNode(ValueConsStrStmt lv, ValueBDDStmt rv)
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.PlusOpWithCons(new Integer(lv.getValues()[0]),
					rv.toValueIntStmt());
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Caanot do aritmatic operation "
					+ "on string values.");
