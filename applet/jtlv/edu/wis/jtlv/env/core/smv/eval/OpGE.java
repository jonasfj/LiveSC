...
public class OpGE extends AbstractBinaryOperator implements IAritOperator {

	public OpGE(AbstractElement a_left, AbstractElement a_right)
	...
	protected ValueStmt do_eval() throws SMVParseException {
		...
		if (l.isValueConsStmt() & r.isValueConsStmt()) {
			...
			if ((l_vals.length != 1) | (r_vals.length != 1))
				throw new SMVParseException("Cannot do GE operator for "
		...
		if (d2c | c2d) {
			...
			try {
				...
				BDD b = d2c ? d.aboveIntToBDD(int_val).or(d.intToBDD(int_val))
						: d.belowIntToBDD(int_val).or(d.intToBDD(int_val));
		...
		if (lc | rc) {
			try {
				...
				ValueIntStmt other = lc ? r.toValueIntStmt() : l
						.toValueIntStmt();
				return lc ? ValueIntStmt.GEOpWithCons(int_val, other)
						: ValueIntStmt.GEOpWithCons(other, int_val);
			} catch (NumberFormatException nfe) {
		...
		return ValueIntStmt.GEOp(l.toValueIntStmt(), r.toValueIntStmt());
	}
	...
	@Override
	protected OpGE duplicate() throws SMVParseException {
		return new OpGE(this.getLeft(), this.getRight());
	}
	...
	protected ValueStmt doDefaultNodeOp(ValueStmt lv, ValueStmt rv)
		...
		return ValueIntStmt.GEOp(lv.toValueIntStmt(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doConsRange2DomNode(ValueConsRangeStmt lv,
		...
		BDD res = rv.belowIntToBDD(int_val).or(rv.intToBDD(int_val));
	...
	protected ValueStmt doConsRange2IntNode(ValueConsRangeStmt lv,
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.GEOpWithCons(lv.fromRange(), rv.toValueIntStmt());
	}
	...
	protected ValueStmt doConsStr2IntNode(ValueConsStrStmt lv, ValueIntStmt rv)
		...
		if (!lv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.GEOpWithCons(new Integer(lv.getValues()[0]), rv
					.toValueIntStmt());
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Cannot do aritmatic operation "
					+ "on string values.");
	...
	protected ValueStmt doDom2ConsRangeNode(ValueDomStmt lv,
		...
		BDD res = lv.aboveEqIntToBDD(int_val);
		return new ValueBDDStmt(lv.getContext(), res);
	}
	...
	protected ValueStmt doDom2ConsStrNode(ValueDomStmt lv, ValueConsStrStmt rv)
		...
		try {
			int int_val = Integer.parseInt(vals[0]);
			BDD res = lv.aboveIntToBDD(int_val).or(lv.intToBDD(int_val));
			return new ValueBDDStmt(lv.getContext(), res);
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Cannot do GE operator for "
					+ "constant which is not an integer.");
	...
	protected ValueStmt doInt2ConsRangeNode(ValueIntStmt lv,
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		return ValueIntStmt.GEOpWithCons(lv.toValueIntStmt(), rv.fromRange());
	}
	...
	protected ValueStmt doInt2ConsStrNode(ValueIntStmt lv, ValueConsStrStmt rv)
		...
		if (!rv.isSingleValue())
			throw new SMVParseException("Cannot do aritmatic operation"
					+ "on constant with other then single value.");
		try {
			return ValueIntStmt.GEOpWithCons(lv.toValueIntStmt(), new Integer(
					rv.getValues()[0]));
		} catch (NumberFormatException nfe) {
			throw new SMVParseException("Cannot do aritmatic operation "
					+ "on string values.");
	...
	protected ValueStmt doBDD2BDDNode(ValueBDDStmt lv, ValueBDDStmt rv)
		...
		BDD res = lv.toBDD().not().and(rv.toBDD()).not();
		return new ValueBDDStmt(lv.getContext(), res);
	}
	...
	protected ValueStmt doBDD2DomNode(ValueBDDStmt lv, ValueDomStmt rv)
			throws SMVParseException {
		// better performance then the default...
		BDD res = rv.belowIntToBDD(1).or(rv.intToBDD(1)).and(lv.toBDD());
		res = res.or(rv.belowIntToBDD(0)).or(rv.intToBDD(0));
		return new ValueBDDStmt(lv.getContext(), res);
	}
	...
	protected ValueStmt doDom2BDDNode(ValueDomStmt lv, ValueBDDStmt rv)
			throws SMVParseException {
		// better performance then the default...
		BDD b = lv.aboveIntToBDD(1).or(lv.intToBDD(1)).and(rv.toBDD());
		b = b.or(lv.aboveIntToBDD(0)).or(lv.intToBDD(0));
	...
	protected ValueStmt doDom2DomNode(ValueDomStmt lv, ValueDomStmt rv)
		...
		try {
			...
			for (int i = 0; i < vals.length; i++) {
				...
				res = res.or(lv.aboveIntToBDD(int_val).or(lv.intToBDD(int_val))
	...
	protected ValueStmt doInt2BDDNode(ValueIntStmt lv, ValueBDDStmt rv)
			throws SMVParseException {
		// better performance then the default...
		BDD res = lv.aboveIntToBDD(1).or(lv.intToBDD(1)).and(rv.toBDD());
		res = res.or(lv.aboveIntToBDD(0)).or(lv.intToBDD(0));
		return new ValueBDDStmt(lv.getContext(), res);
	}
