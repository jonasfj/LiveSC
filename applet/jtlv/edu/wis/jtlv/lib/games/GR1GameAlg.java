 ...
 * In VMCAI, pages 364–380, Charleston, SC, January 2006.
 * </p>
 * <p>
 ...
 * 
 */
public class GR1GameAlg implements GameAlgTwoPlayersI {
	private Module env;
	private Module sys;
	private GeneralizedReactivePair gr;
	public BDD[][][] x_mem;
	public BDD[][] y_mem;
	public BDD[] z_mem;

	public GR1GameAlg(Module env, Module sys, GeneralizedReactivePair gr_pair)
			throws GameAlgException {
		if ((env == null) || (sys == null) || (gr_pair == null)) {
			throw new GameAlgException("cannot instantiate a GR[1] Game with "
					+ "an empty player.");
		}
		this.env = env;
		this.sys = sys;
		...
		x_mem = new BDD[gr.getNumOfGuarantee()][gr.getNumOfAssum()][50];
		y_mem = new BDD[gr.getNumOfGuarantee()][50];
		z_mem = new BDD[gr.getNumOfGuarantee()];
	}
	 ...
	 */
	@Override
	public AlgResultI preAlgorithm() throws GameAlgException {
		// this phase return nothing...
		return null;
	}
	 ...
	 */
	@Override
	public AlgResultI doAlgorithm() throws GameAlgException {
		BDD x, y, z;
		...
		for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
			for (int j = 0; j < gr.getNumOfGuarantee(); j++) {
				cy = 0;
				y = Env.FALSE();
				for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
					BDD start = gr.getGuarantee(j).and(env.yieldStates(sys, z))
							.or(env.yieldStates(sys, y));
					y = Env.FALSE();
					for (int i = 0; i < gr.getNumOfAssum(); i++) {
						BDD negp = gr.getAssum(i).not();
						x = z.id();
	 ...
	 */
	@Override
	public AlgResultI postAlgorithm() throws GameAlgException {
	...
	public AlgResultI calculate_strategy(int kind) {
		...
		while (!st_stack.isEmpty()) {
			...
			int p_i = -1;
			for (int i = 0; i < gr.getNumOfGuarantee(); i++) {
			...
			for (Iterator<BDD> iter_succ = succs.iterator(); iter_succ
				...
				while (candidate.isZero() & (local_kind >= 0)) {
					...
					if ((local_kind == 3) | (local_kind == 7)
						...
						int next_p_j = (p_j + 1) % gr.getNumOfGuarantee();
						if (!p_st.and(gr.getGuarantee(p_j)).isZero()) {
					...
					if ((local_kind == 1) | (local_kind == 6)
						...
						if (!p_st.and(gr.getAssum(p_i).not()).isZero()) {
