...
public class JTLVJavaFactory extends BDDFactoryIntImpl {
	...
	class bddPair extends BDDPairing {
		...
		public bddPair copy() {
			...
			System
					.arraycopy(this.result, 0, THAT.result, 0,
							this.result.length);
			THAT.last = this.last;
			THAT.id = this.id;
			THAT.next = null;
		...
		public String toString() {
			...
			for (int i = 0; i < result.length; ++i) {
				if (result[i] != (ZDD ? zdd_makenode(i, 0, 1)
						: bdd_ithvar(bddlevel2var[i]))) {
					if (any)
						sb.append(", ");
					any = true;
	...
	public void setVarOrder(List domains) {
		...
		for (int i = 0; i < k; ++i) {
			if (parent != getParent(my_vartree[i])) {
				throw new BDDException("var block " + my_vartree[i].firstVar
	...
	private final void DECREF(int node) {
		int rc = bddnodes[node * __node_size + offset__refcou_and_level]
				& REF_MASK;
		if (rc != REF_MASK && rc != 0)
			bddnodes[node * __node_size + offset__refcou_and_level] -= REF_INC;
	}
	...
	private final int LEVEL(int node) {
		return bddnodes[node * __node_size + offset__refcou_and_level]
				& LEV_MASK;
	}
	...
	static final int BDD_RANGE = (-3); /*
										 * Variable value out of range (not in
										 * domain)
										 */
	static final int BDD_DEREF = (-4); /*
										 * Removing external reference to
										 * unknown node
										 */
	static final int BDD_RUNNING = (-5); /*
										 * Called bdd_init() twice without
										 * bdd_done()
										 */
	...
	static final int BDD_DECVNUM = (-15); /*
										 * Trying to decrease the number of
										 * variables
										 */
	static final int BDD_REPLACE = (-16); /*
										 * Replacing to already existing
										 * variables
										 */
	static final int BDD_NODENUM = (-17); /*
										 * Number of nodes reached user defined
										 * maximum
										 */
	...
	int replace_rec(int r) {
		...
		if (ZDD)
			res = zdd_correctify(LEVEL(replacepair[LEVEL(r)]), READREF(2),
					READREF(1));
		else
	...
	int satone_rec(int r) {
		...
		if (ISZERO(LOW(r))) {
			...
			int m = makenode_impl(LEVEL(r), res,
					(ZDD && LOW(r) == HIGH(r)) ? res : BDDZERO);
			PUSHREF(m);
			return m;
		}
	}
	...
	void BddCache_clean_abc(BddCache cache) {
		...
		for (n = 0; n < cache.tablesize; n++) {
			...
			if (a < 0)
				continue;
			if (LOW(a) == -1 || LOW(cache.table[n].b) == INVALID_BDD
	...
	boolean bdd_reorder_ready() {
		if ((bddreordermethod == BDD_REORDER_NONE) || (vartree == null)
				|| (bddreordertimes == 0) || (reorderdisabled != 0))
			return false;
		return true;
	}
	...
	int bdd_setvarnum(int num) {
		...
		for (bdv = bddvarnum; bddvarnum < num; bddvarnum++) {
			if (ZDD) {
				...
				for (int k = num - 1; k >= 0; --k) {
					...
					int res_not2 = (k == bddvarnum) ? res_not : zdd_makenode(k,
							res_not, res_not);
					INCREF(res_not2);
					DECREF(res_not);
					res_not = res_not2;
		...
		if (ZDD) {
			System.out.println("Changed number of ZDD variables to " + num
					+ ", all existing ZDDs are now invalid.");
	...
	void reorder_localGbcResize(int toBeProcessed, int var0) {
		...
		while (toBeProcessed != 0) {
			int next = NEXT(toBeProcessed);
			int hash = NODEHASH2(VARr(toBeProcessed), LOW(toBeProcessed),
					HIGH(toBeProcessed));
	...
	void reorder_swap(int toBeProcessed, int var0) {
		...
		while (toBeProcessed != 0) {
			...
			hash = NODEHASH2(VARr(toBeProcessed), LOW(toBeProcessed),
					HIGH(toBeProcessed));
			SETNEXT(toBeProcessed, HASH(hash));
			SETHASH(hash, toBeProcessed);
	...
	int reorder_makenode(int var, int low, int high) {
		 ...
		 * is no need to add referencerecursively
		 */
	...
	static class LoadHash {
		...
		public LoadHash copy() {
			LoadHash THAT = new LoadHash();
			THAT.key = this.key;
			THAT.data = this.data;
	...
	BddTree bddtree_addrange_rec(BddTree t, BddTree prev, int first, int last,
			boolean fixed, int id) {
	...
	BddTree bddtree_addrange(BddTree t, int first, int last, boolean fixed,
			int id) {
	...
	protected IntBDD makeBDD(/* bdd */int v) {
		if (USE_FINALIZER)
			return new JTLVIntBDDWithFinalizer(v);
		else
	...
	public class JTLVIntBDD extends IntBDD {
		...
		public String toString(Module context) {
			return Env.toNiceSignleLineString(context, this);
		}
	}
	...
	public class JTLVIntBDDWithFinalizer extends JTLVIntBDD {
		...
		protected void finalize() throws Throwable {
			...
			if (false && v != invalid_bdd_impl()) {
				System.out.println("BDD not freed! "
	...
	public JTLVJavaFactory newCloneFactory() {
		...
		if (bddlevel2var != null) {
			...
			System.arraycopy(this.bddlevel2var, 0, INSTANCE.bddlevel2var, 0,
					this.bddlevel2var.length);
		}
		...
		if (bddnodes != null) {
			...
			System.arraycopy(this.bddnodes, 0, INSTANCE.bddnodes, 0,
					this.bddnodes.length);
		}
		...
		if (bddrefstack != null) {
			...
			System.arraycopy(this.bddrefstack, 0, INSTANCE.bddrefstack, 0,
					this.bddrefstack.length);
		}
		...
		if (bddvar2level != null) {
			...
			System.arraycopy(this.bddvar2level, 0, INSTANCE.bddvar2level, 0,
					this.bddvar2level.length);
		}
		INSTANCE.bddvarnum = this.bddvarnum;
		if (bddvarset != null) {
			INSTANCE.bddvarset = new int[this.bddvarset.length];
			System.arraycopy(this.bddvarset, 0, INSTANCE.bddvarset, 0,
					this.bddvarset.length);
		}
		...
		if (extroots != null) {
			...
			System.arraycopy(this.extroots, 0, INSTANCE.extroots, 0,
					this.extroots.length);
		}
		...
		INSTANCE.lh_freepos = this.lh_freepos;
		INSTANCE.lh_nodenum = this.lh_nodenum;
		if (lh_table != null) {
			INSTANCE.lh_table = new LoadHash[this.lh_table.length];
			for (int i = 0; i < this.lh_table.length; i++) {
				INSTANCE.lh_table[i] = this.lh_table[i].copy();
			}
		}
		if (loadvar2level != null) {
			INSTANCE.loadvar2level = new int[this.loadvar2level.length];
			System.arraycopy(this.loadvar2level, 0, INSTANCE.loadvar2level, 0,
					this.loadvar2level.length);
		}
		...
		if (quantvarset != null) {
			...
			System.arraycopy(this.quantvarset, 0, INSTANCE.quantvarset, 0,
					this.quantvarset.length);
		}
		...
		if (replacepair != null) {
			...
			System.arraycopy(this.replacepair, 0, INSTANCE.replacepair, 0,
					this.replacepair.length);
		}
		...
		if (supportSet != null) {
			...
			System.arraycopy(this.supportSet, 0, INSTANCE.supportSet, 0,
					this.supportSet.length);
		}
		...
		if (super.to_free != null) {
			...
			System.arraycopy(super.to_free, 0, INSTANCE.to_free, 0,
					super.to_free.length);
		}
	...
	// /////////////////////////////////////////////////
	// /////////////////////////////////////////////////
	public class JTLVIntBDDVarSet extends IntBDDVarSet {
		protected JTLVIntBDDVarSet(/* bdd */int v) {
			super(v);
		}

		// something is wrong with the original implementation
		...
		public String toString() {
			String res = "<";
			BDDDomain[] doms = this.getDomains();
			if (doms.length == 0)
				return "<empty>";
			res += doms[0].toString();
	...
	public class JTLVIntBDDVarSetWithFinalizer extends JTLVIntBDDVarSet {

		protected JTLVIntBDDVarSetWithFinalizer(int v) {
			super(v);
		}
	...
	public class JTLVIntZDDVarSet extends JTLVIntBDDVarSet {
	...
	public class JTLVIntZDDVarSetWithFinalizer extends JTLVIntZDDVarSet {

		protected JTLVIntZDDVarSetWithFinalizer(int v) {
			super(v);
		}
	...
	protected IntBDDVarSet makeBDDVarSet(/* bdd */int v) {
		if (isZDD()) {
			if (USE_FINALIZER)
				return new JTLVIntZDDVarSetWithFinalizer(v);
			else
				return new JTLVIntZDDVarSet(v);
		} else {
			if (USE_FINALIZER)
				return new JTLVIntBDDVarSetWithFinalizer(v);
			else
				return new JTLVIntBDDVarSet(v);
		}
	}
}
